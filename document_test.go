...
func ExampleDocument_GetWordAfterCursorUntilSeparator() {
	d := &Document{
		Text:           `hello,i am trantaiBD,thank you for using go-prompt`,
...
func ExampleDocument_GetWordBeforeCursorUntilSeparatorIgnoreNextToCursor() {
	d := &Document{
		Text:           `hello,i am trantaiBD,thank you for using go-prompt`,
		cursorPosition: len(`hello,i am trantaiBD,`),
	}
...
func TestDocument_DisplayCursorPosition(t *testing.T) {
	patterns := []struct {
		...
		},
		{
			...
			// must be checked.
			// https://github.com/trantaiBD/go-prompt/pull/99
			document: &Document{
				Text:           "Добрый день",
				cursorPosition: 3,
			},
